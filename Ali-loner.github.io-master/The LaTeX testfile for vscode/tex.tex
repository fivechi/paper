...
\subsection{实验步骤}
\begin{enumerate}
  \item 搭建光路，开启 LED 光源，将分辨率板放在夹具上进行标定。
  \item 配制适当浓度的不同直径、不同液体粘度的聚苯乙烯（PS）微球。
  \item 将溶液加入比色皿中，放在夹具上静置 0.5--1 h。
  \item 开启 LED，观察 PS 微球，待稳定时收集数据。
  \item 将比色皿取下，拍摄背景图片。
  \item 进行数据处理。
\end{enumerate}
\subsection{数据处理}
...
\subsection{实验步骤}
\begin{enumerate}
  \item 使用前面搭建的光路，开启 LED 光源，将分辨率板放在夹具上进行标定。
  \item 配制适当浓度、直径为 500 nm 的聚苯乙烯（PS）微球。
  \item 剪取两片铜片作为电极，使用无水乙醇清洗后晾干。
  \item 将溶液加入比色皿中，放在夹具上，将铜片电极清洗后插入比色皿；连接信号发生器并保证铜片平行紧贴两侧，静置 0.5--1 h。
  \item 开启 LED，观察 PS 微球，待稳定时依次加不同振幅（0--10 V）的电压，收集数据。
  \item 将比色皿取下，拍摄背景图片。
  \item 进行数据处理。
\end{enumerate}
\section{熵产生率估算}
